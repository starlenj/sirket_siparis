<template>
  <div class="col-md-12">
    ...
    <table class="table">
      ...
      <tbody>
        <tr v-for="(item, index) in TunaliData.data" :key="item._id">
          <th>{{ item._id | formatDate }}</th>
          <td>{{ item.credit.toFixed(2) }}</td>
          <td>
            {{
              TunaliData.programData[index] === undefined
                ? 0
                : TunaliData.programData[index]["KREDI"]
            }}
          </td>
          <td>
            {{
          ...
          <tr v-for="(item, index) in TunaliData.data" :key="item._id">
            ...
            <td>
              {{
                TunaliData.programData[index] === undefined
                  ? 0
                  : TunaliData.programData[index]["NAKIT"]
              }}
            </td>
            <td>
              {{
          ...
          <tr v-for="(item, index) in TunaliData.data" :key="item._id">
            ...
            <td>
              {{
                TunaliData.programData[index] === undefined
                  ? 0
                  : TunaliData.programData[index]["YEMEKSEPETI"]
              }}
            </td>
            <td>
              {{
          ...
          <tr v-for="(item, index) in TunaliData.data" :key="item._id">
            ...
            <td>
              {{
                TunaliData.programData[index] === undefined
                  ? 0
                  : TunaliData.programData[index]["YEMEKCEKI"]
              }}
            </td>
            <td>
              {{
        ...
        <tbody>
          <tr v-for="(item, index) in TunaliData.data" :key="item._id">
            ...
            <td>
              {{
                parseFloat(
                  TunaliData.VegaKasa[index] === undefined
                    ? 0
                    : TunaliData.VegaKasa[index].TLGIDER
                ).toFixed(2)
              }}
            </td>
            <td>
...
export default {
  ...
  methods: {
    async GetKasa() {
      ...
      console.log("BAHÇELİ PROGRAM DATA", this.TunaliData);
    },
  },
};
</script>
