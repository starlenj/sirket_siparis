        ...
        <div class="card shadow mb-4">
          ...
          <div class="card-body">
            <div class="form-group">
              <label>Tarih</label>
              <input type="date" class="form-control" v-model="model.date" data-date-format="YYYY MM DD" />
            </div>
          <div class="form-group">
            <label for="">Şube : </label>
            <select class="form-control"  v-model="model.selectSube">
              <option v-for="item in model.data" :key="item._id" v-bind:value="item._id">{{item.name}}</option>
            </select>  
          </div>
      ...
      <div class="col-lg-12">
        <div class="card shadow mb-4">
          ...
          <div class="card-body">
            <table class="table">
              ...
              <tbody>
                <tr v-for="order in model.orderData" :key="order._id">
                  ...
                  <td>
                    <input type="number" class="form-control" v-model="order.onayMiktar" />
                  </td>
                  <td>
                    <input type="text" class="form-control" v-model="order.onayAciklama"  />
                  </td>
                  <td>
            ...
            <button class="btn btn-primary" style="float:right" @click="SiparisOnayla()">Vegaya Aktar</button>
              <button class="btn btn-primary" @click="SiparisOnayla()">Onayla</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    async SiparisGetir() {
      var date = moment(this.model.date).format("YYYY.MM.DD");
      var subeId = this.model.selectSube;
    ...
    async SiparisOnayla() {
      this.model.orderData.forEach(async order => {
        ...
        let result = await SepetService.SepetOnay(order);
        if (result) {
